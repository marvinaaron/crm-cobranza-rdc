  ...
  const CartBody = (
    <>
      <div className="flex items-center justify-between gap-2 mb-3">
        ...
        <button
          type="button"
          className="lg:hidden text-[11px] font-bold text-slate-500 hover:text-slate-800"
        ...
        <div className="relative p-4 sm:p-5 flex flex-col h-full">
          <div className="flex items-center justify-between gap-2">
            {destacado ? (
              ...
              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-slate-50 text-[9px] font-bold uppercase tracking-wider text-slate-500 ring-1 ring-slate-200">
                Solución personalizada
              </span>
            )}
          </div>

          <h3 className="mt-3 text-[15px] sm:text-base font-black leading-snug text-slate-900">
            {paq.nombre}
          </h3>
          ...
          {esPublico ? (
            <div className="mt-3">
              <div className="flex items-baseline gap-1.5">
                <span className="text-3xl sm:text-4xl font-black tracking-tight tabular-nums text-slate-900">
          ...
          <ul className="mt-3 space-y-1.5 flex-1">
            {visibles.map((l) => (
              <li
                key={l}
                className="flex items-start gap-2 text-[10px] text-slate-600 leading-snug"
              >
                <svg
                  width="14"
                  height="14"
          ...
          <button
            ...
            disabled={on}
            className={`mt-4 inline-flex items-center justify-center gap-1.5 h-10 rounded-xl text-[11px] font-bold transition w-full active:scale-[0.98] ${
              on
      ...
      <div className="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_340px] gap-5 lg:gap-8 items-start">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-stretch">
            <div className="rounded-3xl bg-white/70 ring-1 ring-slate-200/80 p-4 sm:p-5 flex flex-col">
              ...
              <ul className="flex-1 grid grid-cols-1">
          ...
          <div
            ...
            className={`space-y-6 scroll-mt-24 ${
              mostrarConfig ? "" : "hidden lg:block lg:opacity-95"
            }`}
          >
            <div className="rounded-2xl bg-white ring-1 ring-slate-200/80 shadow-sm p-4 sm:p-5">
              <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500 mb-1 text-center">
                1 · Cuéntanos de ti
              </p>
              ...
              {regimenesLista.length > 0 && (
                <div className="mt-5 pt-4 border-t border-slate-100">
                  ...
                  <p className="text-[11px] text-slate-500 text-center mb-3">
                    Toca el que aplica · o déjalo en “Aún no lo sé”
                  </p>
                  <div className="flex flex-wrap justify-center gap-2">
                    <button
                      type="button"
                      onClick={() => setRegimen("__skip__")}
                    ...
                    {regimenesLista.map((r) => {
                      const activo = regimenPillValue === r.id;
                      return (
                        <button
                          key={r.id}
                          type="button"
