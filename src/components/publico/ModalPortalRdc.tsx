  ...
  return (
    <>
      {!expandidoMovil ? (
        <button
          type="button"
          onClick={() => setExpandidoMovil(true)}
          className="fixed z-[240] bottom-4 left-4 sm:hidden flex h-14 w-14 items-center justify-center rounded-full bg-[radial-gradient(120%_90%_at_50%_-10%,#f5f3ff_0%,#ede9fe_38%,#e9d5ff_68%,#faf5ff_100%)] shadow-[0_18px_50px_-8px_rgba(15,23,42,0.28)] ring-1 ring-violet-200/70"
          aria-label="Abrir Portal RDC"
        >
          <Logo mark="r" variante="black" alto={24} className="relative" />
          <span className="absolute -top-1 -right-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-red-500 px-1 text-[10px] font-bold text-white ring-2 ring-white dark:ring-black">
            1
          </span>
        </button>
      ) : null}
      ...
      <aside
        ...
        className={`fixed z-[240] bottom-4 left-4 right-4 sm:right-auto sm:w-[22.5rem] overflow-hidden rounded-[1.75rem] bg-white dark:bg-black shadow-[0_18px_50px_-8px_rgba(15,23,42,0.28)] ring-1 ring-violet-200/70 dark:ring-white/10 animate-in fade-in slide-in-from-bottom-4 duration-300 ${
          expandidoMovil ? "block" : "hidden sm:block"
        }`}
      >
        <button
          type="button"
