...
/**
 * Toggle para activar/desactivar notificaciones push en el portal del cliente.
 * Se puede usar en la pantalla de perfil del portal.
 */
export default function PushToggle() {
  ...
  const handleDesactivar = async () => {
    setTrabajando(true);
    setMensaje(null);
    const ok = await desactivarPushParaCliente();
    setTrabajando(false);
    if (ok) {
      setEstado("inactivo");
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-500 mt-1 leading-relaxed">
            Recibe avisos de facturas, pagos validados y vencimientos en tu
            celular o computadora aunque no tengas el portal abierto.
          </p>
        </div>
        <span
      ...
      <div className="mt-4">
        {estado === "no-soportado" ? (
          <p className="text-xs text-rose-700 leading-relaxed">
            Este navegador no soporta notificaciones push. Te recomendamos usar Chrome,
            Edge, Firefox o Safari recientes. En iPhone, instala el portal en la
            pantalla de inicio para activarlas.
          </p>
        ) : estado === "denegado" ? (
          <p className="text-xs text-rose-700 leading-relaxed">
            Las notificaciones están bloqueadas para este sitio. Abre los permisos del
            navegador y permite las notificaciones para volver a activarlas.
          </p>
        ) : estado === "activo" ? (
          <button
            type="button"
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold bg-indigo-600 text-white hover:bg-indigo-700 transition-colors disabled:opacity-50"
          >
